...
def Dijkstra(graph, start_node, end_node):
    ...
    """
    paths = {v: np.inf for v in graph.nodes} # [0:0, 2:3
    prev_nodes = {v: np.inf for v in graph.nodes}

    paths[start_node] = 0
    prev_nodes[start_node] = 0

    nodes_visited = [start_node] # [0, 3, 2, 5]

    while end_node not in nodes_visited:
        current_node = nodes_visited[-1] # 0
        neighbours = graph[current_node] # 2, 3, 4
        ...
        # if using a different data structure, use set
        # valid_nodes = set(paths) - set(nodes_visited)
